	...
	calculate_model_matrix(actor, sim_time, actors_by_name) {
		...
		*/
		...
		if(actor.orbit !== null) {
			...
			const translate_to_radius = mat4.fromTranslation(mat4.create(), [actor.orbit_radius, 0., 0.])
			const orbit_rotation = mat4.fromZRotation(mat4.create(), sim_time * actor.orbit_speed + actor.orbit_phase)

			mat4_matmul_many(M_orbit, translate_to_parent, orbit_rotation, translate_to_radius)
			// Orbit around the parent
		} 
		
		const M_spin = mat4.fromZRotation(mat4.create(), sim_time * actor.rotation_speed)
		const M_scale = mat4.fromScaling(mat4.create(), [actor.size, actor.size, actor.size])
