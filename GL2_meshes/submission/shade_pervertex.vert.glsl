...
void main() {
	...
	*/
	...
	vec3 dir_to_light = normalize(light_position - vertex_view);

	vertex_to_fragment = material_ambient * material_color * light_color;
	if(dot(normal_view, dir_to_light) > 0.){
		vertex_to_fragment += light_color * material_color * dot(normal_view, dir_to_light);
		if(dot(normal_view, halfway_vect) > 0.){
			vertex_to_fragment += light_color * material_color * pow(dot(halfway_vect, normal_view), material_shininess);
		}
	}
	+ light_color * material_color * dot(normal_view, dir_to_light) + light_color * material_color * pow(dot(halfway_vect, normal_view), material_shininess);
