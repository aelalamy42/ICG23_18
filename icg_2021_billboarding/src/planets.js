	...
	calculate_model_matrix({sim_time}) {
		...
		*/
		...
		if(this.orbits !== null) {
			...
			const translate_to_parent = mat4.fromTranslation(mat4.create(), parent_translation_v)
			const translate_to_radius = mat4.fromTranslation(mat4.create(), [this.orbit_radius, 0., 0.])
			const orbit_rotation = mat4.fromZRotation(mat4.create(), sim_time * this.orbit_speed + this.orbit_phase)

			mat4_matmul_many(M_orbit, translate_to_parent, orbit_rotation, translate_to_radius)
			// Orbit around the parent
		} 
		
		const M_spin = mat4.fromZRotation(mat4.create(), sim_time * this.rotation_speed)
		const M_scale = mat4.fromScaling(mat4.create(), [this.size, this.size, this.size])
		

		
		// Store the combined transform in actor.mat_model_to_world
		mat4_matmul_many(this.mat_model_to_world, M_orbit, M_spin, M_scale)
...
export class MeshOrbitActor extends PhongActor {
	init_pipeline(regl, resources) {
		// create pipeline only if it doesn't exist
		// if pipeline not found under that key, the arrow-function is used to create it
		this.pipeline = cached_pipeline('phong_mesh', () => regl({
			// Vertex attributes
			attributes: {
